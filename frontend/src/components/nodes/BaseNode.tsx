...
export function BaseNode({ id, data }: { id: string; data: PlutoNodeData }) {
	...
	const Icon =
		(LucideIcons as Record<string, any>)[iconName] || LucideIcons.Code;
		...
		<div className="pluto-node">
			...
			>
				...
				<div className="pluto-node__title-group">
					<span className="pluto-node__label">
						{data.label as string}
					</span>
					<span className="pluto-node__category">
						{categoryLabel}
					</span>
				</div>
				<div className="pluto-node__status-wrap">
					{data.status === 'running' && (
						<LucideIcons.Loader2
							size={12}
							className="pluto-node__status-icon pluto-node__status--running"
						/>
					)}
					{data.status === 'done' && (
						<LucideIcons.Check
							size={12}
							className="pluto-node__status-icon pluto-node__status--done"
						/>
					)}
					{data.status === 'error' && (
						<LucideIcons.AlertCircle
							size={12}
							className="pluto-node__status-icon pluto-node__status--error"
						/>
					)}
					<button
				...
				<div className="pluto-node__ports">
					<div className="pluto-node__port-col pluto-node__port-col--left">
						{data.inputs?.map((input) => (
							<div
								key={input.id}
								className="pluto-node__port-row"
							>
								<Handle
									type="target"
									...
									style={{
										background: `var(--handle-${input.type}, var(--handle-any))`,
									}}
								/>
								<span
									className="pluto-node__port-label pluto-node__port-label--left"
									title={input.label}
								>
									{input.label}
									{input.label && input.optional && (
										<span className="pluto-node__port-optional">
											?
										</span>
									)}
								</span>
					...
					<div className="pluto-node__port-col pluto-node__port-col--right">
						{data.outputs?.map((output) => (
							<div
								key={output.id}
								className="pluto-node__port-row pluto-node__port-row--right"
							>
								<span
									className="pluto-node__port-label pluto-node__port-label--right"
									title={output.label}
								>
									{output.label}
								</span>
								<Handle
									...
									style={{
										background: `var(--handle-${output.type}, var(--handle-any))`,
									}}
								/>
							</div>
						))}
			...
			{(data.configSchema?.length ?? 0) > 0 && (
				<div className="pluto-node__config nodrag">
					{data.configSchema.map((field) => (
						<div
							key={field.key}
							className="pluto-node__field"
						>
							<label className="pluto-node__field-label">
								{field.label}
							</label>
							...
							{field.type === 'string' && (
								<input
									type="text"
									value={
										(data.config[field.key] as string) ?? ''
									}
									onChange={(e) =>
										updateNodeConfig(id, {
