...
export const nodeRegistry: Record<string, NodeTypeDefinition> = {
	...
	variable: {
		...
		generateCode: (node, _inputs, outputVar) =>
			`${outputVar} = ${node.data.config.value || 'None'}`,
		imports: [],
		pipPackages: [],
	},
	...
	print_debug: {
		...
		generateCode: (node, inputs) => {
			const lbl = node.data.config.label
	...
	if_condition: {
		...
		category: 'primitive',
		description:
			'Compare two values and route program execution through the true or false path',
		icon: 'GitBranch',
		inputs: [
	...
	for_loop: {
		...
		category: 'primitive',
		description:
			'Run a small Python body once per item and collect each result into a list',
		icon: 'Repeat',
		inputs: [{ id: 'iterable', label: 'iterable', type: 'list' }],
		outputs: [{ id: 'results', label: 'results', type: 'list' }],
		configSchema: [
			{
				key: 'body',
				label: 'Body (use item, index, and result)',
				type: 'code',
				default: 'result = item',
			},
		],
	...
	file_write: {
		...
		generateCode: (node, inputs) => {
			const fp = inputs['filepath']
				? inputs['filepath']
				: `"${node.data.config.filepath}"`;
	...
	df_groupby: {
		...
		configSchema: [
			...
			},
			{
				...
				label: 'Aggregation',
				type: 'select',
				options: ['mean', 'sum', 'count', 'max', 'min'],
				default: 'mean',
			},
		],
	...
	df_sort: {
		...
		generateCode: (node, inputs, outputVar) => {
			const asc =
				node.data.config.ascending === 'false' ||
				node.data.config.ascending === false
					? 'False'
					: 'True';
	...
	model_trainer: {
		...
		outputs: [
			{ id: 'trained_model', label: 'trained model', type: 'model' },
		],
		configSchema: [],
		defaultConfig: {},
	...
	model_predict: {
		...
		outputs: [
			{ id: 'predictions', label: 'predictions', type: 'dataframe' },
		],
		configSchema: [],
		defaultConfig: {},
		generateCode: (_node, inputs, outputVar) =>
	...
	cross_validator: {
		...
		configSchema: [
			{ key: 'cv', label: 'CV Folds', type: 'number', default: 5 },
		],
		defaultConfig: { cv: 5 },
		generateCode: (node, inputs, outputVar) =>
	...
	hyperparameter_tuner: {
		...
		generateCode: (node, inputs, outputVar) => {
			...
			const cls =
				search === 'random' ? 'RandomizedSearchCV' : 'GridSearchCV';
	...
	nn_linear: {
		...
		generateCode: (node, _inputs, outputVar) => {
			...
			const act =
				activation === 'none'
					? ''
					: `, nn.${activation === 'relu' ? 'ReLU' : activation === 'sigmoid' ? 'Sigmoid' : 'Tanh'}()`;
	...
	nn_sequential: {
		...
		configSchema: [
			{
				key: 'layers_config',
				label: 'One layer per line',
				type: 'textarea',
				default: 'Linear(128, 64)\nReLU()\nLinear(64, 10)',
			},
		],
		defaultConfig: {
			layers_config: 'Linear(128, 64)\nReLU()\nLinear(64, 10)',
		},
		generateCode: (node, _inputs, outputVar) =>
			`${outputVar} = nn.Sequential(\n    nn.${String(
				node.data.config.layers_config || 'Linear(128, 10)',
			)
				.split('\n')
				.filter(Boolean)
	...
	training_loop: {
		...
		generateCode: (node, inputs, outputVar) => {
			...
			const lossFn =
				loss === 'cross_entropy' ? 'CrossEntropyLoss' : 'MSELoss';
			return [
				`import numpy as np`,
				`${outputVar} = ${inputs['model'] || 'model'}`,
	...
	model_infer: {
		...
		imports: ['import torch', 'import numpy as np'],
		pipPackages: ['torch', 'numpy'],
	},
	...
	llm_call: {
		...
		generateCode: (node, inputs, outputVar) => {
			const { provider, model, api_key_env, temperature } =
				node.data.config;
	...
	prompt_template: {
		...
		configSchema: [
			{
				key: 'template',
				label: 'Template ({var} syntax)',
				type: 'textarea',
				default: 'Answer this question: {question}',
			},
		],
	...
	confusion_matrix: {
		...
		pipPackages: ['scikit-learn', 'plotly'],
	},
	...
	roc_auc: {
		...
		imports: ['from sklearn.metrics import roc_curve, auc'],
		pipPackages: ['scikit-learn', 'plotly'],
	},
	...
	df_preview: {
		...
		configSchema: [
			{ key: 'rows', label: 'Rows to Show', type: 'number', default: 10 },
		],
	...
	agg_stat: {
		...
		configSchema: [
			...
			},
			{
				...
				label: 'Statistic',
				type: 'select',
				options: ['mean', 'median', 'std', 'min', 'max', 'count'],
				default: 'mean',
			},
		],
